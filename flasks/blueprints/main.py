...
from ..utils import check_final_url, get_claude_login_url, getoauth, login_required, register_token
...
def submit_name():
    ...
    if Type == 'chatgpt':
        ...
        if index and 1 <= index <= len(valid_tokens):
            ...
            if 'http' in access_token or 'https' in access_token:
                old_url = "https://plus.aivvm.com/"
                new_url = "https://gpt.bocchi2b.top/"
                
                # 替换旧的 URL
                new_access_token = new_access_token.replace(old_url, new_url)

                url = new_access_token  # 你要测试的原始链接

                target_url = 'https://gpt.bocchi2b.top/auth/login_auth0'  # 你要匹配的目标地址
                logurl = check_final_url(url, target_url)
